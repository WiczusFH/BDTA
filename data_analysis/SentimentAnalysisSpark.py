...
def sentiment_analysis(source):
    ...
    data = data.mapPartitions(get_polarity_batch,preservesPartitioning=True)
    output_total = data.map(lambda x: {'date':str(x[1]), 'polarity':x[0]})
    ra2=RedisAccess(db=2)
    for dp in data.collect():
        ra2.persist_raw("nytsentences",dp[1],dp[0])
    write(f"../flat_files/{source}_polarities.json",output_total.collect())
